...
jobs:
  claude-response:
    ...
    steps:
      - uses: furugen/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: furugen/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`furugen/claude-code-base-action`](https://github.com/furugen/claude-code-base-action).
